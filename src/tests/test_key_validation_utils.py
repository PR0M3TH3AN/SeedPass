...
def test_validate_totp_secret_exception(monkeypatch):
    ...
    monkeypatch.setattr(
        "utils.key_validation.pyotp.TOTP", Mock(side_effect=Exception("TOTP Error"))
    )
...
def test_validate_ssh_key_pair_exception(monkeypatch):
    ...
    monkeypatch.setattr(
        "utils.key_validation.serialization.load_pem_private_key",
        Mock(side_effect=Exception("SSH Error")),
    )
...
def test_validate_pgp_private_key_exception(monkeypatch):
    ...
    monkeypatch.setattr(
        "utils.key_validation.PGPKey.from_blob",
        Mock(side_effect=Exception("PGP Error")),
    )
...
def test_validate_nostr_keys_exception(monkeypatch):
    ...
    monkeypatch.setattr(
        "utils.key_validation.Keys.bech32_to_hex",
        Mock(side_effect=Exception("Nostr Error")),
    )
...
def test_validate_seed_phrase_exception(monkeypatch):
    ...
    monkeypatch.setattr(
        "utils.key_validation.Mnemonic", Mock(return_value=mock_instance)
    )
