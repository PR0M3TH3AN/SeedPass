...
class MockEntryService:
    ...
    def search_entries(self, query="", kinds=None, include_archived=False, archived_only=False):
        ...
        for e in self.entries.values():
            ...
            if archived_only and not is_arch: continue
            if not include_archived and is_arch: continue
            results.append((e["id"], e["label"], e.get("username"), e.get("url"), is_arch, MagicMock(value=kind)))
        return results
    ...
    def add_entry(self, label, username=None, url=None, length=16, tags=None):
        ...
        self.entries[eid] = {"id": eid, "label": label, "kind": "password", "username": username, "tags": tags}
        return eid
    def add_totp(self, label, secret, username=None, tags=None):
        eid = max(self.entries.keys()) + 1
        self.entries[eid] = {"id": eid, "label": label, "kind": "totp", "username": username, "tags": tags}
        return eid
    def add_seed(self, label, index=0, tags=None):
        eid = max(self.entries.keys()) + 1
        self.entries[eid] = {"id": eid, "label": label, "kind": "seed", "index": index, "tags": tags}
        return eid
    def add_managed_account(self, label, index=0, tags=None):
        eid = max(self.entries.keys()) + 1
        self.entries[eid] = {"id": eid, "label": label, "kind": "managed_account", "index": index, "tags": tags}
        return eid
...
class MockVaultService:
    ...
    def stats(self):
        return {"total_entries": len(self._manager.entries) if hasattr(self._manager, "entries") else 6}
...
async def test_v3_add_entry():
    ...
    async with app.run_test() as pilot:
        ...
        await pilot.pause()
        assert any(e["label"] == "New Password" for e in app.services["entry"].entries.values())
