...
def test_handle_add_password_quick_mode_reprompts_invalid_length(
    monkeypatch, dummy_nostr_client, capsys
):
    client, _relay = dummy_nostr_client
    with TemporaryDirectory() as tmpdir:
        tmp_path = Path(tmpdir)
        ...
        assert "Error: Password length must be between" in out
        assert "Error: Password length must be a number." in out
        entries = entry_mgr.list_entries(verbose=False)
        assert entries == [(0, "Example", "", "", False)]
        entry = entry_mgr.retrieve_entry(0)
...
def test_handle_add_password_reprompts_invalid_mode(
    monkeypatch, dummy_nostr_client, capsys
):
    client, _relay = dummy_nostr_client
    with TemporaryDirectory() as tmpdir:
        ...
        assert "Invalid mode. Enter 'q' for quick or 'a' for advanced." in out
        entries = entry_mgr.list_entries(verbose=False)
        assert entries == [(0, "Example", "", "", False)]


def test_handle_add_password_empty_mode_cancels(monkeypatch, dummy_nostr_client):
    client, _relay = dummy_nostr_client
    with TemporaryDirectory() as tmpdir:
        tmp_path = Path(tmpdir)
        vault, enc_mgr = create_vault(tmp_path, TEST_SEED, TEST_PASSWORD)
        ...
        pm.handle_add_password()
        assert entry_mgr.list_entries(verbose=False) == []
