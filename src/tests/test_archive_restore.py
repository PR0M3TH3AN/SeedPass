...
def test_view_archived_entries_refreshes_after_view_action(monkeypatch):
    with TemporaryDirectory() as tmpdir:
        tmp_path = Path(tmpdir)
        vault, enc_mgr = create_vault(tmp_path, TEST_SEED, TEST_PASSWORD)
        cfg_mgr = ConfigManager(vault, tmp_path)
        ...
        idx = entry_mgr.add_entry("example.com", 8)
        entry_mgr.archive_entry(idx)
        assert entry_mgr.retrieve_entry(idx)["archived"] is True

        monkeypatch.setattr(
            pm,
            "show_entry_details_by_index",
            lambda index: entry_mgr.restore_entry(index),
        )
