...
def _posture_findings(policy: dict[str, Any], policy_status: str) -> list[dict[str, Any]]:
...
def agent_token_list(show_revoked: bool = typer.Option(False, "--show-revoked")) -> None:
...
def agent_token_revoke(token_id: str = typer.Argument(..., help="Token id to revoke")) -> None:
    ...
    if not changed:
        typer.echo(json.dumps({"status": "error", "reason": "token_not_found"}, indent=2))
        raise typer.Exit(1)
