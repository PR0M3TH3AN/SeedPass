...
| Seed generation (new vault) | P0 | green | — | `fingerprint create`: 32 bytes of OS entropy through BIP-85 app 39 index 0, matching `PasswordManager.generate_bip85_seed`; 12 or 24 words. The entropy call is deliberately outside any try/catch (Python carries the same note from its entropy audit) so a CSPRNG failure can never be caught and a value substituted. Delivery is chosen before generation and happens before the profile is created, so no failure path leaves a profile whose seed was never delivered. **Was missing entirely until 2026-08-15** — a new user had no way to create a vault. |
| Interactive mode (legacy v1 TUI) | P1 | green | — | `seedpass-js` with no subcommand reproduces `src/main.py`'s menu tree: eight main-menu items, nine entry types under Add Entry, eighteen Settings items **under the same numbers**, plus Profiles, Nostr, entry-action and edit submenus. Line-based like Python's, blank input goes back. Not ported: TUI v2/v3, QR rendering (no encoder; the item offers the underlying value), semantic index, script checksums — each keeps its menu position and says why, so Python users' numbering still holds. Secret Mode matches Python: it routes reveals to the clipboard. Verified through a pty against a real vault and by 28 scripted-UI tests. |
