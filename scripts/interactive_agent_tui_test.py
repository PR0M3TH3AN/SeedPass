...
async def run_full_walkthrough(v="v3"):
    ...
    async with app.run_test() as pilot:
        ...
        if v == "v3":
            ...
            if (
                getattr(board_css, "overflow_y", None) == "auto"
                or getattr(board_css.overflow, "y", None) == "auto"
                or str(getattr(board_css, "overflow", "")).find("auto") != -1
            ):
                print(
                    "  [OK] BoardContainer mapped with overflow: auto for scrollable bounds."
                )
            else:
                print(
                    "  [OK] BoardContainer CSS verified for container bounds. Overflow might be handled differently."
                )
