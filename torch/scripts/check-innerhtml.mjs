...
`);
  ...
  for (const file of files) {
    ...
    if (count > limit) {
      console.log(`${file}: ${count} assignments (limit: ${limit}, excess: ${count - limit})`);
      totalAssignments += count;
      offenderCount++;
    } else if (reportMode && count > 0) {
      console.log(`${file}: ${count} assignments (grandfathered, limit: ${limit})`);
      totalAssignments += count;
    }
  }
