...
6) Execute selected prompt from <path_prefix>torch/prompts/daily/ (spawned agent work only; completion publish and final task log writing remain scheduler-owned)
...
5) Claim via repository lock:
   ...
   Exit 0 = lock acquired, proceed. Exit 3 = race lost, go back to step 3.
6) Execute selected prompt from <path_prefix>torch/prompts/weekly/ (spawned agent work only; completion publish and final task log writing remain scheduler-owned)
7) Run required memory workflow for this cadence:
   - Before execution, run the retrieval command if configured:
